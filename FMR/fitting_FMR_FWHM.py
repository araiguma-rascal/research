    ...
    for filename in files:
        if '.csv' in filename:
            data = pandas.read_csv(
                path + filename, usecols=cols, header=0, skiprows=lambda x: x in range(1, 551), skipfooter=220, engine='python')
...
def main():
    # get data
    obj_array = get_data('../', ['Magnetic Field (kG)', 'S21 (dB)'])
    ...
    result = pandas.DataFrame(index=[], columns=[
                              'Temperature (K)', 'Gate Voltage (V)', 'Sign', 'a0 (1)', 'a1 (mT^-1)', 'a2 (mT^-2)', 'H_res (mT)', 'FWHM (mT)', 'Amp', 'b'])
    ...
    for i, obj in enumerate(obj_array):
        ...
        obj.data['S21 (1)'] = np.power(10, obj.data['S21 (dB)'] / 20)
        obj.sign = 'Negative' if '_r.csv' in obj.filename else 'Positive'

        # fitting
        estimation_ini = [
            3, 0, 0, -36, 2, 1, 1] if '_r.csv' in obj.filename else [3, 0, 0, 35.5, 2, 1, 1]
        try:
            params = fitting(
                fmr, obj.data['Magnetic Field (mT)'], obj.data['S21 (1)'], estimation_ini)
        except RuntimeError:
            print("Optimization Failed!!!")
            params = estimation_ini
        obj.data['S21_fit (1)'] = fmr(obj.data['Magnetic Field (mT)'], *params)
        ...
        ax.plot(obj.data['Magnetic Field (mT)'], obj.data['S21 (1)'])
        ax.plot(obj.data['Magnetic Field (mT)'], obj.data['S21_fit (1)'])

    print(result)
    data2csv(result, './', 'result.csv')
    plt.show()
...
# function for FMR
def fmr(x, a0, a1, a2, H_res, FWHM, Amp, b):
    return a0 + a1*x + a2*x**2 + Amp * (1+b*(x-H_res)/FWHM) / ((x - H_res)**2 + (FWHM/2)**2)
